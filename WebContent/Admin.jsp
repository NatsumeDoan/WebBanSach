					...
					<c:forEach items="${dskh }" var="xn">
						...
						<tr>
							...
							<td data-th="Trạng thái">
								<c:if test="${xn.isDamua()==false}">
									<div class="col-md-9 text-left mt-sm-2">
										<h4 style="margin-top: 30px">Dang dat mua</h4>
									</div>
								</c:if> 
								<c:if test="${xn.isDamua()}">
									<div class="col-md-9 text-left mt-sm-2">
										<h4 style="margin-top: 30px">Da thanh toan</h4>
									</div>
								</c:if>
							</td>
							<td><a href="xacnhanchuyentiencontroller?MaHoaDon=${xn.getMaHoaDon() }&MaChiTietHD=${xn.getMaChiTietHD() }">
									Xác nhận đã chuyển tiền </a></td>
					</c:forEach>
